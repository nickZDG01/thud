...
\section{Metriche}
Le vulnerabilità di un sistema informatico sono classificate in base alla loro gravità e quindi al pericolo che esse possono comportare a tale sistema, in modo da poter prioritizzare gli interventi di risouzione.

Il livello di \textit{severity} è calcolato secondo le seguenti metriche e viene espresso attraverso il CVSS \textit{score}:

\begin{itemize}
    \item 
\end{itemize}

\footnote{Il \textit{Common Vulnerability Scoring System}, originariamente sviluppato dal \textit{National Infrastructure Advisory Council}, è un sistema standardizzato che assegna un punteggio da zero a dieci (severità più alta) alle vulnerabilità}
